package gpsdraw.springboots.config.security;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        httpSecurity.httpBasic().disable() // REST API는 UI를 사용하지 않으므로 기본설정을 비활성화
            ...
            .antMatchers(HttpMethod.GET, "/product/**","https://localhost:8080/**").permitAll() // product로 시작하는 Get 요청은 허용
    ...
    public void configure(WebSecurity webSecurity) {
        webSecurity.ignoring().antMatchers("/v3/api-docs/**",
                ...
                "/auth/**",
                "/sign-api/exception",
                "/swagger-resources/**", // 추가: Swagger 리소스에 대한 접근 허용
                "/webjars/**"
        );
    }
}